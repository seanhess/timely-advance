...
testIncomeUntil = do
  test "weekly" $ do
    ...
    let income = BudgetInfo "" (Weekly Monday) (absolute $ Money.fromFloat 100.00)
    Needed.incomeUntil today due income @?= Money.fromFloat 400.00

  test "monthly, on same date, do not include" $ do
    let today = parseDay "2019-02-01" -- friday
    let due   = parseDay "2019-03-01" -- friday
    let income = BudgetInfo "" (Monthly (DayOfMonth 1)) (absolute $ Money.fromFloat 100.00)
...
testDueDates = do
  group "monthly rent, monthly paychecks" $ do
    ...
    let income = BudgetInfo "" (Monthly (DayOfMonth 1)) (absolute payAmount)
    let bill = BudgetInfo "" (Monthly (DayOfMonth 5)) (absolute $ Money.fromFloat 1000.00)
  ...
  group "weekly bill, monthly paycheck" $ do
    ...
    let income = BudgetInfo "" schedule (absolute payAmount)
    let bill = BudgetInfo "" (Weekly Monday) (absolute $ Money.fromFloat 100.00)
    test "just paid" $ do
